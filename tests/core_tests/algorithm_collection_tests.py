...
class AlgorithmCollectionTests(unittest.TestCase):
    ...
    def test_empty_catalog(self):
        with self.assertRaises(RuntimeError) as error:
            AlgorithmCollection(self.path_config, ALGORITHM_CONFIG, LOG_CONFIG_STUB)
    ...
    def test_single_algorithm(self):
        ...
        algorithms = AlgorithmCollection(
            self.path_config, ALGORITHM_CONFIG, LOG_CONFIG_STUB
        )
        self.assertEqual(algorithms.get_name_title_dict(), {"test_single": FIB_TITLE})
        alg = algorithms.get_algorithm(name)
        self.assertIsInstance(alg, Algorithm)
        self.assertEqual(alg.name, name)
        self.assertEqual(
            algorithms.get_algorithm_result(name, {"n": 20}), {"result": 6765}
        )
    ...
    def test_double_algorithms(self):
        ...
        self.assertEqual(
            algorithms.get_name_title_dict(), {"test1": FIB_TITLE, "test2": FIB_TITLE}
        )
    ...
    def test_triple_algorithms(self):
        ...
        self.assertEqual(
            algorithms.get_name_title_dict(),
            {"test1": FIB_TITLE, "test2": FIB_TITLE, "test3": FIB_TITLE},
        )
    ...
    def test_algorithm_not_exists(self):
        ...
        self.assertRaisesRegex(
            ValueError,
            ALGORITHM_NOT_EXISTS_TEMPL.format(wrong_name),
            algorithms.get_algorithm_result,
            wrong_name,
            {"n": 20},
        )
