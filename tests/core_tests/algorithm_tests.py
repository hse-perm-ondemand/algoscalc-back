...
class AlgorithmTests(unittest.TestCase):
    ...
    output = DataElement(
        OUTPUT_NAME, OUTPUT_TITLE, OUTPUT_DESCRIPTION, DataType.INT, DataShape.SCALAR, 0
    )
    output_matrix = DataElement(
        OUTPUT_MATRIX_NAME,
        OUTPUT_TITLE,
        OUTPUT_DESCRIPTION,
        DataType.INT,
    ...
    def setUp(self) -> None:
        ...
        self.output_matrix = DataElement(
            OUTPUT_MATRIX_NAME,
            OUTPUT_TITLE,
            OUTPUT_DESCRIPTION,
            DataType.INT,
    ...
    def test_non_string_name(self):
        ...
        self.assertEqual(NON_STRING_PARAM_TEMPL.format(NAME), str(error.exception))
    ...
    def test_empty_string_name(self):
        ...
        self.assertEqual(EMPTY_STRING_PARAM_TEMPL.format(NAME), str(error.exception))

    def test_non_string_title(self):
        with self.assertRaises(ValueError) as error:
            Algorithm(NAME, 1.1, "description", LOG_CONFIG_STUB)
        self.assertEqual(NON_STRING_PARAM_TEMPL.format("title"), str(error.exception))
    ...
    def test_empty_string_title(self):
        ...
        self.assertEqual(EMPTY_STRING_PARAM_TEMPL.format("title"), str(error.exception))

    def test_non_string_description(self):
        with self.assertRaises(ValueError) as error:
            Algorithm(NAME, "title", 1, LOG_CONFIG_STUB)
        self.assertEqual(
            NON_STRING_PARAM_TEMPL.format("description"), str(error.exception)
        )
    ...
    def test_empty_string_description(self):
        ...
        self.assertEqual(
            EMPTY_STRING_PARAM_TEMPL.format("description"), str(error.exception)
        )
    ...
    def test_add_non_data_element_parameter(self):
        self.assertRaisesRegex(
            TypeError, PARAM_NOT_DATAELEMENT_MSG, self.alg.add_parameter, "param"
        )
    ...
    def test_add_duplicate_parameter(self):
        ...
        self.assertRaisesRegex(
            ValueError,
            PARAM_EXISTS_TMPL.format(PARAM_NAME),
            self.alg.add_parameter,
            self.param,
        )
    ...
    def test_add_non_data_element_output(self):
        self.assertRaisesRegex(
            TypeError, OUTPUT_NOT_DATAELEMENT_MSG, self.alg.add_output, "output"
        )
    ...
    def test_add_execute_method_non_callable(self):
        self.assertRaisesRegex(
            TypeError, METHOD_NOT_CALL_MSG, self.alg.add_execute_method, "method"
        )
    ...
    def test_add_execute_method_timeout(self):
        ...
        param_default = 0
        param = DataElement(
            param_name, "param_title", "param_descr", DataType.INT, DataShape.SCALAR, 0
        )
        ...
        timeout_msg = TIME_OVER_TEMPL.format(timeout, {param_name: param_default})
    ...
    def test_zero_execute_timeout(self):
        ...
        alg = Algorithm(NAME, TITLE, DESCRIPTION, LOG_CONFIG_STUB, timeout)
        param = DataElement(
            "param_val", "param_title", "param_descr", DataType.INT, DataShape.SCALAR, 0
        )
        alg.add_parameter(param)
        alg.add_output(self.output)
    ...
    def test_add_execute_method_without_parameters(self):
        ...
        self.assertEqual(
            str(error.exception), ADDING_METHOD_FAILED_TEMPL.format(UNSET_PARAMS_MSG)
        )
    ...
    def test_add_execute_method_without_outputs(self):
        ...
        with self.assertRaises(Exception) as error:
            self.alg.add_execute_method(lambda param: {OUTPUT_NAME: param})
        self.assertEqual(
            str(error.exception), ADDING_METHOD_FAILED_TEMPL.format(UNSET_OUTPUTS_MSG)
        )
    ...
    def test_add_execute_method_wrong_param(self):
        ...
        err = ADDING_METHOD_FAILED_TEMPL.format(
            EXECUTION_FAILED_TEMPL.format(
                UNEXPECTED_PARAM_MSG, {PARAM_NAME: PARAM_INT_DEFAULT}
            )
        )
        with self.assertRaises(RuntimeError) as error:
            self.alg.add_execute_method(lambda wrong_name: {"output": wrong_name})
    ...
    def test_add_execute_method_missing_param(self):
        ...
        self.alg.add_parameter(self.param_list)
        self.alg.add_output(self.output)
        params = {PARAM_NAME: PARAM_INT_DEFAULT, PARAM_LIST_NAME: PARAM_LIST_DEFAULT}
        err = ADDING_METHOD_FAILED_TEMPL.format(
            EXECUTION_FAILED_TEMPL.format(UNEXPECTED_PARAM_MSG, params)
        )
        with self.assertRaises(Exception) as error:
            self.alg.add_execute_method(lambda param_name: {OUTPUT_NAME: param_name})
        self.assertEqual(str(error.exception), err)
    ...
    def test_add_execute_method_wrong_output(self):
        ...
        with self.assertRaises(Exception) as error:
            self.alg.add_execute_method(lambda param_name: {wrong_name: param_name})
        self.assertEqual(str(error.exception), err)
    ...
    def test_add_execute_method_wrong_output_value(self):
        ...
        wrong_value = -1
        err = ADDING_METHOD_FAILED_TEMPL.format(
            UNEXPECTED_OUTPUT_TEMPL.format(OUTPUT_NAME, wrong_value, PARAM_INT_DEFAULT)
        )
        with self.assertRaises(Exception) as error:
            self.alg.add_execute_method(lambda param_name: {OUTPUT_NAME: wrong_value})
        self.assertEqual(str(error.exception), err)
    ...
    def test_execute_redundant_param(self):
        ...
        with self.assertRaises(KeyError) as error:
            self.alg.execute(
                {PARAM_NAME: PARAM_INT_DEFAULT, redundant_param: PARAM_INT_DEFAULT}
            )
        self.assertEqual(
    ...
    def test_execute_non_dict_output(self):
        ...
        self.alg.add_execute_method(
            lambda param_name: (
                ...
                if param_name == PARAM_INT_DEFAULT
                else param_name
            )
        )
        with self.assertRaises(TypeError) as error:
            self.alg.execute({PARAM_NAME: -1})
        self.assertEqual(str(error.exception).strip("'"), NOT_DICT_OUTPUTS_MSG)
    ...
    def test_execute_runtime_error(self):
        param = DataElement(
            "param_val", "param_title", "param_descr", DataType.INT, DataShape.SCALAR, 1
        )
    ...
    def test_execute(self):
        alg = Algorithm("sum", "sum", "returns the sum of two numbers", LOG_CONFIG_STUB)
        param_a = DataElement(
            "a", "a number", "just an integer", DataType.INT, DataShape.SCALAR, 1
        )
        param_b = DataElement(
            "b", "b number", "just an integer", DataType.INT, DataShape.SCALAR, 2
        )
        alg.add_parameter(param_a)
        alg.add_parameter(param_b)
        output = DataElement(
            "sum", "sum", "the sum of two numbers", DataType.INT, DataShape.SCALAR, 3
        )
