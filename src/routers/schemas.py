...
class AlgorithmsPageSchema(BaseModel):
    ...
    items: list[DefinitionSchema] = Field(
        ..., description="Список объектов на текущей странице"
    )
...
class PaginateInputSchema(BaseModel):
    ...
    page: int = Field(
        1, ge=1, description="Номер страницы, должен быть больше или равен 1"
    )
    size: int = Field(
        50,
        ge=1,
        le=100,
        description="Количество объектов на странице, должно быть от 1 до 100",
    )
